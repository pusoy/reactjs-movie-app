...
const Genre = () => { 
    const [state, dispatch] = useReducer(reducer, { genreList: [], popularList: [], count: 1 })
    ...
    useEffect(() => {
        ...
        const getMovieList = async () => {
            try {
                ...
                const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${Config().API}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate&with_genres=${genre}&page=${state.count}`)
            ...
            <div className="genre-detail">
                {
                    state.genreList.map((res) => {
                        ...
                        return (
                            <Link className={genre == res.id ? `genre-box is-genre-active` : `genre-box`} to={genreLink} key={res.id} onClick={() => { setGenre(res.id) } } >{res.name}</Link>
                        ) 
                    })
                }  
            </div> 
